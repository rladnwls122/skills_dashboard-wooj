...
5. **시험 (Sandbox)** — WAF 규칙을 적용 전에 **로컬에서만** 돌려본다. AWS로
   아무것도 보내지 않고 WebACL도 건드리지 않는다. 아래 "규칙 시험 샌드박스" 참고.
6. **규칙생성 (AI)** — 용도별(의심 경로 / 의심 User-Agent / SQL 인젝션)로 정규식
   패턴 세트 규칙을 따로 조립하고, 패턴·근거·판단 기준·규칙 JSON을 함께 보여준다.
   **"시험 탭으로 보내기"**를 누르면 그 JSON이 시험 탭 편집기에 채워진다.
