...
백엔드와 실행 스크립트가 시작할 때 이 파일을 **직접 읽습니다** — mise·direnv·dotenv-cli 같은 런처가 없어도 됩니다.
...
- 이미 export 된 환경변수는 `.env` 가 덮지 않습니다. `API_ADDR=… pnpm start` 로 그때만 다르게 띄울 수 있습니다.
- 다른 위치의 파일을 쓰려면 `ENV_FILE=경로`.
