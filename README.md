...
5. **보고 (Incident)** — "Generate Incident Context" 버튼으로 현재까지 수집된
   ...
   - **Amazon Q 프롬프트** (기본값) — Q의 프롬프트 입력 한도인 10,000자에 맞춰
     ...
     (아래 참고)이라 Q가 404/403 증가를 장애로 오판하지 않는다. `[F]`에는
     **Top 경로 10개**(각 경로에 `[지정]`/`[미지정]` 표시)와 **Top
     User-Agent 8개**가 요청순으로 들어가, Q가 스캐너·정찰 패턴을 직접
     짚을 수 있다. 규칙 JSON 본문·Pod 로그 원문·전체 타임라인은 빠지며,
     잘린 항목은 말미에 이름으로 남는다. 글자 수는 셀렉트 옆에 표시되고
     한도를 넘으면 빨간색.
