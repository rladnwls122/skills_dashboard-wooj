...
`-p`(또는 `--port`)로 UI 포트를 지정하면 두 번째 인스턴스를 나란히 띄울 수 있습니다. `pnpm dev`도 같은 플래그를 받습니다.

```bash
pnpm start            # UI 3100 · API 8787 · ./data/dashboard.db
pnpm start -p 3110    # UI 3110 · API 8797 · ./data/dashboard-3110.db
```

API 포트는 UI 포트에서 자동 파생되고(3100→8787 오프셋 유지), 기본 포트가 아니면 SQLite 파일도 포트별로 분리됩니다 — 두 백엔드가 하나의 SQLite 파일을 두고 잠금을 다투거나 설정·자격증명을 공유하는 일을 막기 위한 것입니다. `PORT`·`API_ADDR`·`API_PROXY_TARGET`·`DB_PATH` 환경 변수로 각각 재정의할 수 있습니다.

### AWS 자격증명 자동 불러오기

백엔드는 시작할 때 설정 화면의 "CLI 불러오기" 버튼과 같은 동작을 한 번 수행합니다: 이전 실행에서 저장(persist)해 둔 키도 없고 환경 변수 키도 없으면, 로컬 `aws` CLI의 기본 프로파일(`AWS_PROFILE` 반영)을 세션 한정으로 불러옵니다. 실패해도 서버는 그대로 뜨고 로그 한 줄만 남으며, 설정 탭에서 언제든 직접 불러올 수 있습니다.
