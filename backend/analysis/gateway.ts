// The gateway response contract and the Amazon Q prompt packer. A 404 on /login
// is the gateway working, a 404 on /v1/user is a routing failure — every report
// states the contract up front.

import { appTrafficPaths, isAppTrafficPath } from "../config/paths.ts";
import type { HttpSummary, PathStat } from "../../src/lib/types.ts";
...
/**
 * Renders the contract for the top of a report. Five lines: this is the
 * interpretation key, not the finding.
 */
export function contractLines(): string[] {
  const c = GATEWAY_CONTRACT;
  return [
    "- 지정 경로: " + appTrafficPaths().join(", "),
...
/**
 * Scores the sampled traffic against the contract. Deliberately hedged: WAF logs
 * carry the WAF's own action, not the status the client finally saw.
 */
export function evaluateContract(h: HttpSummary | null): ContractCheck {
  const check: ContractCheck = { conforming: [], deviations: [] };
  if (!h) return check;

  const c = GATEWAY_CONTRACT;
  const listed: PathStat[] = [];
  const unlisted: PathStat[] = [];
  for (const p of h.byPath) {
    if (p.lowPriority) continue;
    if (isAppTrafficPath(p.path)) listed.push(p);
  ...
  const topN = (list: PathStat[], n: number, f: (p: PathStat) => string): string =>
    list.slice(0, n).map(f).join(", ");

  if (unlistedPassed.length > 0) {
    const passedCount = unlistedPassed.reduce((acc, p) => acc + p.count - p.blocked, 0);
    check.deviations.push(
      `미지정 경로 ${unlistedPassed.length}개(요청 ${passedCount}건)가 WAF를 통과 — 게이트웨이가 ${c.unlistedStatus}로 응답했는지 확인 필요 (${c.normalStatus}면 라우팅 노출): ` +
        topN(unlistedPassed, 5, (p) => `${p.path} ${p.count - p.blocked}건 통과`),
    );
  }
  if (unlistedBlocked.length > 0) {
    // A WAF Block answers 403 by default. On an unlisted path that tells the
    // scanner the path is guarded — the contract wants it indistinguishable from
    ...
    check.deviations.push(
      `미지정 경로가 WAF에서 차단됨 → 기본 응답은 ${c.abnormalStatus}이지만 계약상 ${c.unlistedStatus}여야 함. 해당 규칙에 CustomResponse ${c.unlistedStatus} 설정 검토: ` +
        topN(unlistedBlocked, 5, (p) => `${p.path} ${p.blocked}건`),
    );
  }
  if (listedBlocked.length > 0) {
    const total = listedBlocked.reduce((acc, p) => acc + p.blocked, 0);
    check.conforming.push(
      `지정 경로 차단 ${total}건 → 비정상 요청에 대한 ${c.abnormalStatus}로 계약과 일치 (` +
        topN(listedBlocked, 5, (p) => `${p.path} ${p.blocked}건`) +
        ")",
    );
  }
  ...
  if (d) {
    ...
    if (d.c3xx > 0) {
      check.deviations.push(`3XX ${d.c3xx}건/분 — 계약에 없는 응답. 리다이렉트 경로 확인 필요.`);
    }
    if (d.c4xx > 0 && unlisted.length > 0) {
      const total = unlisted.reduce((acc, p) => acc + p.count, 0);
      check.conforming.push(
        `4XX ${d.c4xx}건/분 관측 + 미지정 경로 요청 ${total}건 — 스캐닝에 대한 ${c.unlistedStatus} 응답으로 설명 가능. 4XX 증가만으로 장애로 판정하지 말 것.`,
      );
    }
    if (d.c5xx === 0 && d.c3xx === 0) {
      check.conforming.push(
        `5XX/3XX 없음 — 응답 코드는 계약 범위(${c.normalStatus}/${c.abnormalStatus}/${c.unlistedStatus}) 안에 있음.`,
      );
    }
  }
 ...
 * and everything after it is named but not included.
 */
export function packToLimit(header: string[], sections: QSection[], limit: number): string {
  ...
  if (omitted.length > 0) {
    out.push(
      "",
      `> 길이 제한(${limit}자)으로 생략된 항목: ${omitted.join(", ")}. 전체 내용은 Markdown/JSON 산출물 참조.`,
    );
  }
